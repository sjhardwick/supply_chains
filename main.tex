...
\begin{equation} \label{eq:welfare}
    \mathbb{E} \left[ W_j \right] = \left( \sum_{s \in S} \mathbb{P}(s) \left[ U_j(s) + \frac{R_j(s) - Z_j(s) + \Pi_{j}(s)}{\bar{P}_{Fj}} \right]^{1 - \gamma} \right)^{\frac{1}{1 - \gamma}} .
\end{equation}
$R_j(s)$ is total revenue from any import tariffs and $Z_j(s)$ the total cost of any production or entry subsidies. $\Pi_j(s)$ are total profits from the input-producing sector in country $j$ ?(the final goods sector makes zero profit). $\gamma$ is the coefficient of relative risk aversion. The agent is risk averse if $\gamma > 0$.
